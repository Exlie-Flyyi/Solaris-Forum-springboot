...
public class WebMvcConfig implements WebMvcConfigurer {
    ...
    @Autowired
    private LoginTicketInterceptor loginTicketInterceptor;//注入拦截器
    ...
    // 对除静态资源外所有路径进行拦截（所有页面的都要进行处理）
    @Override
    public void addInterceptors(InterceptorRegistry registry) { //注册接口
        registry.addInterceptor(loginTicketInterceptor)
                .excludePathPatterns("/css/**", "/js/**", "/img/**", "/editor-md/**", "/editor-md-upload/**");//排除的路径
